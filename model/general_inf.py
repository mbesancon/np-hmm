...
def mPi(alpha_pi,exp_z,K):
    ...
    exp_z: expectation of latent variables 
    (we are only interested at time step 0 here)
...
def mA(alpha_a,exp_s,K):
    ...
    #K: truncation param. for DP
    tau_a0,tau_a1 = np.zeros((K,K)), np.zeros((K,K))
    for i in range(K):
        for j in range(K):
...
def eFowardsZ(exp_ln_pi,exp_ln_a,ln_obs_lik,N,K):
    ln_alpha_exp_z = np.zeros((N,K)) - np.inf
...
def eBackwardsZ(exp_ln_pi,exp_ln_a,ln_obs_lik,N,K):
    ln_beta_exp_z = np.zeros((N,K)) - np.inf
    #final state distribution:
    ln_beta_exp_z[N-1,:] = np.zeros(K)
...
def eS(exp_ln_a, ln_alpha_exp_z, ln_beta_exp_z, ln_obs_lik, N, K):
    ln_exp_s = np.zeros((N-1,K,K)) #we only care about the transitions, which is why the length of this var is (N-1)
