...
def viterbiLog(ln_lik_obs,exp_a,exp_pi,VERBOSE=False):
    ...
    (given the observations) using the Viterbi algorithm"""

    (N,K) = np.shape(ln_lik_obs)
    
    #initialize T1 and T2
    T1,T2 = np.zeros((N,K)), np.zeros((N,K))
    T1[0,:] = np.log(exp_pi) + np.ln_lik_obs[0,:]
    ...
    for n in range(1,N):
        for s in range(K):
            ...
            v = T1[n-1,:] + np.log(exp_a[:,s]) + ln_lik_obs[n,s]
        ...
        #normalise:
        T1[n,:] -= T1[n,:].max()
        T1[n,:] = np.log(np.exp(T1[n,:])/np.exp(T1[n,:]).sum())
